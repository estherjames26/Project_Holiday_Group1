# Shows what's saved in the local database — past searches and API cache.
...
from database import get_cache_stats, get_recommendation_history, get_session, init_db
from page_styling import inject_styles, render_hero

init_db()
inject_styles()
render_hero()

st.header("History & cache")
st.markdown("Data stored locally in `data/holiday_planner.db`.")
...
| `places_cache` | Google Places (bars, restaurants, clubs) |
| `cost_cache` | Scraped trip costs (Numbeo + Cheapflights) |
