# Core logic — loops through destinations, filters bad fits, scores the rest.
# If you're debugging wrong rankings, start here.
...
from src.services.scraper_service import NUMBEO_SLUGS, CostScraperService
...
class RecommendationEngine:
    ...
    def recommend(
        ...
        portfolio = generate_portfolio_insights(output, prefs.__dict__)
    ...
    def _score_destination(
        ...
        slug = NUMBEO_SLUGS.get(dest.id, dest.name.replace(" ", "-"))
        costs = self.costs.get_costs(dest.id, slug, dest.airport_code)
