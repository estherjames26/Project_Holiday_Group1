...
from places import get_destinations
...
def generate_portfolio_insights(
    ...
    total_candidates = candidate_count if candidate_count is not None else len(get_destinations())
    filtered_count = total_candidates - len(results)
    if filtered_count > 0:
